...
fun GalaxyBackground(content: @Composable () -> Unit) {
    Box(modifier = Modifier.fillMaxSize().background(
        Brush.verticalGradient(listOf(Color(0xFF020617), Color(0xFF0F172A), Color(0xFF020617)))
    )) {
            ...
            item {
                Column(modifier = Modifier.padding(vertical = 20.dp)) {
                    Text(
                        ...
                        style = TextStyle(shadow = Shadow(Color.White.copy(0.5f), blurRadius = 10f))
                    )
                    Text(text = "9 رمضان 1447", color = Color.Gray, fontSize = 16.sp)
                }
            }
